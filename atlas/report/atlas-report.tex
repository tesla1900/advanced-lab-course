...
\section{QCD Scale Factor and Cut Selection}
...
In our analysis, we set the scale factor to be 0.22. Fig. \ref{fig:etiso_qcd} shows the \texttt{el\_etiso} distribution in the full range 
...
\begin{figure*}[htb!]
	...
	\quad
    \centering
	\subfloat[Full range, after scaling]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd22.pdf}}}
	\quad
    \centering
	\subfloat[High QCD background region, after scaling]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd22_1020.pdf}}}
	\quad
	\caption{\texttt{el\_etiso} distribution before scaling (top) and after scaling (bottom) for the whole distribution (left)
    and in the region with high QCD background (right). The QCD scale factor was set to 0.22.}
	\label{fig:etiso_qcd}
\end{figure*}
...
and the simulated data was significant (by eye). From this we, chose that the scale factor is acceptable between 0.20 and 
0.25. See Appendix \ref{chap:appendix_figs} for the distributions in the full range 
...
\begin{figure*}[htb!]
	\centering
	\subfloat[No cuts]{{\includegraphics[width=0.47\columnwidth]{ptw_qcd22.pdf}}}
	\quad
	\centering
	\subfloat[After cuts, \texttt{njet == 0, el\_etiso $<$ 8}]{{\includegraphics[width=0.47\columnwidth]{ptw_qcd22_etiso8_njet0.pdf}}}
...
an example, shown in Fig \ref{fig:hm_m80}. For this analysis, we used the fitting range of [26, 55].
...
\begin{table}
    ...
    $0.5209 \pm 0.0075$ & $1.21 \pm 0.35$  & 7.06  \\ \hline
    \end{tabular}
...
As the degrees of freedom is $ndof = 7 - 2 = 5$, we observe that the reduce chi-squared $\chi^2 / ndof = 1.41$, which is close to 1, 
...
we obtain the half-maximum to be $h = 43.157 \pm 0.052$ GeV. See Fig. \ref{fig:hm_atlas} for the electron $p_T$ spectrum with the ATLAS data. 
...
\begin{equation}
    m_W = \SI{80.4 \pm 1.47}{\giga\electronvolt} 
\end{equation}
...
half-maximum fit, and $\sigma_{ab}$ are the correlation coefficients from the covariance matrix of the fit as shown in Eq. \ref{eq:cov_matrix}.
The error is purely statistical, and systematic uncertainties must be considered. We observe that our result is within one sigma of the 
 the most recent value of the W boson of $80.433 \pm 0.009 \ \text{GeV}$ \cite{CDF:2022hxs}. \textcolor{red}{explain why statistical 
 uncertainty is large?}\par
...
\begin{equation}
    m_Z = 90.63 \pm 1.58 GeV
\end{equation}
which is within one sigma of the literature value of the Z boson mass of $\SI{91.1876 \pm 0.0021}{\giga\electronvolt}$. 
...
\begin{figure*}[htb!]
	\centering
	\subfloat[Full range, Scale Factor = 0.2]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd20.pdf}}}
	\quad
	\centering
	\subfloat[High QCD background region, Scale Factor = 0.2]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd20_1020.pdf}}}
	\quad
    \centering
	\subfloat[Full range, Scale Factor = 0.25]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd25.pdf}}}
	\quad
    \centering
	\subfloat[High QCD background region, Scale Factor = 0.25]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd25_1020.pdf}}}
	\quad
	\caption{\texttt{el\_etiso} distribution for the whole distribution (left)
    and in the region with high QCD background (right) for QCD scale factors of 0.20 (top) and 0.25 (bottom).}
	\label{fig:etiso_qcd_unc}
\end{figure*}

\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.9\textwidth]{hw_plots_1.pdf}
