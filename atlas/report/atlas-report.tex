...
In our analysis, we set the scale factor to be 0.26. Fig. \ref{fig:etiso_qcd} shows the \texttt{el\_etiso} distribution in the full range 
...
\begin{figure*}[htb!]
	...
	\quad
    \centering
	\subfloat[Full range, after scaling]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd26.pdf}}}
	\quad
    \centering
	\subfloat[High QCD background region, after scaling]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd26_1020.pdf}}}
	\quad
	\caption{\texttt{el\_etiso} distribution before scaling (top) and after scaling (bottom) for the whole distribution (left)
    and in the region with high QCD background (right). The QCD scale factor was set to 0.26.}
	\label{fig:etiso_qcd}
\end{figure*}
...
and the simulated data was significant (by eye). From this we, chose that the scale factor is acceptable between 0.24 and 
0.28. See Appendix \ref{chap:appendix_figs} for the distributions in the full range 
...
\begin{figure*}[htb!]
	\centering
	\subfloat[No cuts]{{\includegraphics[width=0.47\columnwidth]{ptw_qcd26.pdf}}}
	\quad
	\centering
	\subfloat[After cuts, \texttt{njet == 0, el\_etiso $<$ 8}]{{\includegraphics[width=0.47\columnwidth]{ptw_qcd26_etiso8_njet0.pdf}}}
...
\begin{table}
    ...
    $0.5218 \pm 0.0073$ & $1.03 +- 0.59$  & 7.39  \\ \hline
    \end{tabular}
...
As the degrees of freedom is $ndof = 7 - 2 = 5$, we observe that the reduce chi-squared $\chi^2 / ndof = 1.48$, which is close to 1, 
...
\begin{equation}
    m_W = \SI{80.41 \pm 0.10}{\giga\electronvolt} 
\end{equation}
...
Here, the error is purely statistical. We observe that our result is $0.261\sigma$ from the 
...
\texttt{Zee}. We obtained the half-maximum $h = \SI{48.413 \pm 0.061}{\giga\electronvolt}$, shown in Fig. \ref{fig:hm_zee}. From this, 
we obtain the mass of the Z-boson as such: 
\begin{equation}
    m_Z = \SI{90.89 \pm 0.18}{\giga\electronvolt}
\end{equation}
which is $1.64\sigma$ from the literature value of the Z boson mass of $\SI{91.1876 \pm 0.0021}{\giga\electronvolt}$. Thus the 
...
\begin{equation}
    \frac{\sigma_{m_Z, calib}}{m_Z} = \frac{\sigma_{m_W, calib}}{m_W} \implies \sigma_{m_W, calib} = m_W \frac{\sigma_{m_Z, calib}}{m_Z}
\end{equation} 
...
expected value: $\sigma_{m_Z, calib} = \left| 90.89 - 91.1876 \right| = \SI{0.2976}{\giga\electronvolt}$. From this, we obtain the 
systematic error from calibration for the W-boson mass as such: 
\begin{equation}
    \sigma_{m_W, calib} = (\SI{80.41}{\giga\electronvolt}) \cdot \left( \frac{\SI{0.2976}{\giga\electronvolt}}{\SI{90.89}{\giga\electronvolt}}\right) = \SI{0.26}{\giga\electronvolt}
\end{equation}
Here, we keep the significant digits to two decimal places to match those of the statistical errors. 
...
The QCD scale factors, as mentioned in Sec. \ref{sec:qcdscale_cuts} were chosen to be 0.24 and 0.28 for the minimal and maximal scale 
factor respectively. Table \ref{tab:qcd_syserr} shows the W-boson masses obtained from using the two scale factors. The half-maxima obtained for each 
scale factor for each simulated W-boson mass is shown in Table  in Appendix \ref{chap:appendix_figs}. \textcolor{red}{edit appendix 
referencing for each figure (and code)!}

\begin{table}
    \centering
    \begin{tabular}{|c|c|} \hline
    QCD Scale Factor &  $m_W / \si{\giga\electronvolt}$\\ \hline
    0.24 & $1.03 +- 0.59$ \\ 
    0.28 & $jlfsa$ \\ \hline
    \end{tabular}
    \caption{Parameters obtained from fitting the gauge curve.}
...
To then obtain the systematic uncertainty




% \texttt{el\_pt} tells us the transverse 
% momentum of the electron, which is the relevant distribution for the Jacobi peak. The parameters  \texttt{el\_etiso} and \texttt{njet} 
% are the transverse energy $E_T$ within a jet and the number of jets in the data, which are relevant for 
...
\begin{figure*}[htb!]
	\centering
	\subfloat[Full range, Scale Factor = 0.24]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd24.pdf}}}
	\quad
	\centering
	\subfloat[High QCD background region, Scale Factor = 0.24]{{\includegraphics[width=0.47\columnwidth]{etiso_qcd24_1020.pdf}}}
	\quad
    \centering
    ...
    and in the region with high QCD background (right) for QCD scale factors of 0.24 (top) and 0.28 (bottom).}
