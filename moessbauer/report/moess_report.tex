...
offset $\delta_v$ from a full symmetric distribution was also determined through this fitting process. The equation used to fit
is shown as such:
\begin{equation}
    I_{tot}(v, v_{0_i}, I_{0_i}, \Gamma_{v_i}, \delta_v) = \delta_v + \sum \limits_{i}^{N_{\mathrm{peaks}}} - I_{0_i} \frac{(\Gamma_{v_i} / 2)^2}{(v - v_{0_i})^2 + (\Gamma_{v_i} / 2)^2}
...
The offset was determined to be $\delta_v = \SI{3.131 \pm 0.020}{\milli\meter\per\second} $. The uncertanties were obtained from the 
...
\begin{equation}
    E = E_0 (1 + \frac{v}{c}) \implies E_\gamma = \Delta E = E_0\frac{v}{c}
\end{equation}
where $E_0$ is the expected ground state energy $E_0 = \SI{14.4}{\kilo\electronvolt}$. Table shows the energy value $E_{0_i}$ and its 
linewidth $\Gamma_i$ for each observed peak. 
